...
def decode_agent_tool_calls(message: Any, actions: Sequence[ToolSpec]) -> list[AgentAction]:
    """把原生 function call 消息解析成多个结构化动作。

    主循环支持一次返回多个 tool_call（例如 DeepSeek 一次发起多个 read），
    每个动作都会独立通过 schema 校验，随后由调用方逐个执行。
    """
    ...
    decoded_actions: list[AgentAction] = []
    for tool_call in tool_calls:
        ...
        decoded_actions.append(validate_agent_payload(payload, actions, raw_response))
...
def decode_agent_tool_call(message: Any, actions: Sequence[ToolSpec]) -> AgentAction:
    ...
    如果模型一次返回多个动作，本函数只返回第一个；需要并行执行的调用方
    应使用 decode_agent_tool_calls()。
    """
    decoded = decode_agent_tool_calls(message, actions)
    if len(decoded) > 1:
        names = ", ".join(action.tool for action in decoded[1:])
        print(f"[compat] 模型一次返回 {len(decoded)} 个动作，单动作接口只取第一个，"
              f"其余（{names}）未在本轮执行。")
    return decoded[0]
